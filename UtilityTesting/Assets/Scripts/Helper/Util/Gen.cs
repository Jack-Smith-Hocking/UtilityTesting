...
namespace Jack.Utility
{
    public static partial class Util
    {
        ...
        {
            /// <summary>
            /// Throw an 'ArgumentNullException if the obj is null
            /// </summary>
            ...
            /// </summary>
            public static bool IsNotEmpty<T>(ICollection<T> collection) => collection.Count > 0;
            ...
            /// </summary>
            /// <param name="toPoint">Conversion to Vector3</param>
            public static TPoint GetClosest<TPoint>(TPoint p1, TPoint p2, TPoint anchorPoint, System.Func<TPoint, Vector3> toPoint)
            {
                Vector3 _anchorPoint = toPoint.Invoke(anchorPoint);

                float _p1Dist = Util.Math.DistanceSqrd(toPoint.Invoke(p1), _anchorPoint);
                float _p2Dist = Util.Math.DistanceSqrd(toPoint.Invoke(p2), _anchorPoint);

                return _p1Dist < _p2Dist ? p1 : p2;
            }
            ...
            /// </summary>
            /// <param name="toPoint">Conversion to Vector3</param>
            public static TPoint GetClosestPoint<TPoint>(TPoint point, ICollection<TPoint> points, System.Func<TPoint, Vector3> toPoint)
            {
                return SortClosestPoints(point, points, toPoint).ElementAt(0);
            }
            ...
            /// </summary>
            /// <param name="toPoint">Conversion to Vector3</param>
            public static ICollection<TPoint> SortClosestPoints<TPoint>(TPoint point, ICollection<TPoint> points, System.Func<TPoint, Vector3> toPoint)
            {
                Vector3 _point = toPoint.Invoke(point);

                ICollection<TPoint> _points = points.Sort((a, b) =>
                {
                    float _aDist = Util.Math.DistanceSqrd(toPoint.Invoke(a), _point);
                    float _bDist = Util.Math.DistanceSqrd(toPoint.Invoke(b), _point);

                    return _aDist.CompareTo(_bDist);
                });
